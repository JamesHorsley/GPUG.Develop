...
namespace GPUG.Develop
{
    ...
    /// </summary>
    public class Lesson_3c : IDexterityAddIn
    {
        private PmVendorMaintenanceForm vendorForm = Dynamics.Forms.PmVendorMaintenance;

        public void Initialize()
        {
            vendorForm.AddMenuHandler(updateVendorData, "Update Vendor Data", null);
        }


        // creates a new vendor record
        private void updateVendorData(object sender, EventArgs e)
        {
            ...
            if (error == TableError.NoError)
            {
                ...
                row.VendorContact.Value = "Josh Pelkola";
                row.Address1.Value = "10001 Reunion St";
                row.Address2.Value = "Suite 400";
                row.City.Value = "San Antonio";
                row.State.Value = "TX";
                row.ZipCode.Value = "78216";

                // save the record, and check for errors
                if (row.Save() != TableError.NoError)
